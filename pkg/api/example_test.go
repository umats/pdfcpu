...
func ExampleValidateFile() {
	...
	ValidateFile(context.Background(), "in.pdf", nil, nil)
}
...
func ExampleOptimizeFile() {
	...
	OptimizeFile(context.Background(), "in.pdf", "out.pdf", conf, nil)
	...
	OptimizeFile(context.Background(), "in.pdf", "", nil, nil)
}
...
func ExampleTrimFile() {
	...
	TrimFile(context.Background(), "in.pdf", "outFile", []string{"odd"}, nil)

	// Create a trimmed version of in.pdf containing the first two pages only.
	// If you want to modify the original file, pass an empty string for outFile.
	TrimFile(context.Background(), "in.pdf", "", []string{"1-2"}, nil)
}
...
func ExampleSplitFile() {
	...
	SplitFile(context.Background(), "in.pdf", "outDir", 1, nil)

	// Create dual page PDFs for in.pdf in outDir using the default configuration.
	SplitFile(context.Background(), "in.pdf", "outDir", 2, nil)

	// Create a sequence of PDFs representing bookmark sections.
	SplitFile(context.Background(), "in.pdf", "outDir", 0, nil)
}
...
func ExampleMergeCreateFile() {
	...
	MergeCreateFile(context.Background(), inFiles, "out.pdf", false, nil)
}
...
func ExampleMergeAppendFile() {
	...
	inFiles := []string{"in1.pdf", "in2.pdf"}
	MergeAppendFile(context.Background(), inFiles, "out.pdf", false, nil)
}
...
func ExampleInsertPagesFile() {
	...
	InsertPagesFile(context.Background(), "in.pdf", "", []string{"3"}, true, nil, nil)

	// Insert a blank page into in.pdf after every page.
	InsertPagesFile(context.Background(), "in.pdf", "", nil, false, nil, nil)
}
...
func ExampleRemovePagesFile() {
	...
	RemovePagesFile(context.Background(), "in.pdf", "", []string{"2", "8"}, nil)

	// Remove first 2 pages of in.pdf.
	RemovePagesFile(context.Background(), "in.pdf", "", []string{"-2"}, nil)

	// Remove all pages >= 10 of in.pdf.
	RemovePagesFile(context.Background(), "in.pdf", "", []string{"10-"}, nil)
}
...
func ExampleAddWatermarksFile() {
	...
	onTop := false
	update := false
	wm, _ := TextWatermark(context.Background(), "Demo", "", onTop, update, types.POINTS, nil)
	AddWatermarksFile(context.Background(), "in.pdf", "", nil, wm, nil)
	...
	onTop = true
	wm, _ = TextWatermark(context.Background(), "Confidential", "font:Courier, points:48, col: 1 0 0, rot:45, scale:1 abs, ", onTop, update, types.POINTS, nil)
	AddWatermarksFile(context.Background(), "in.pdf", "", []string{"odd"}, wm, nil)

	// Add image stamps to in.pdf using absolute scaling and a negative rotation of 90 degrees.
	wm, _ = ImageWatermark(context.Background(), "image.png", "scalefactor:.5 a, rot:-90", onTop, update, types.POINTS, nil)
	AddWatermarksFile(context.Background(), "in.pdf", "", nil, wm, nil)
	...
	wm, _ = PDFWatermark(context.Background(), "stamp.pdf:2", "scale:.5 abs, diagonal:2", onTop, update, types.POINTS, nil)
	AddWatermarksFile(context.Background(), "in.pdf", "", nil, wm, nil)
}
...
func ExampleRemoveWatermarksFile() {
	...
	update = true
	wm, _ = TextWatermark(context.Background(), "Confidential", "", onTop, update, types.POINTS, nil)
	AddWatermarksFile(context.Background(), "in.pdf", "", nil, wm, nil)

	// Add another watermark on top of page 1
	wm, _ = TextWatermark(context.Background(), "Footer stamp", "c:.5 1 1, pos:bc", onTop, update, types.POINTS, nil)
	AddWatermarksFile(context.Background(), "in.pdf", "", nil, wm, nil)

	// Remove watermark on page 1
	RemoveWatermarksFile(context.Background(), "in.pdf", "", []string{"1"}, nil)

	// Remove all watermarks
	RemoveWatermarksFile(context.Background(), "in.pdf", "", nil, nil)
}
...
func ExampleImportImagesFile() {
	...
	ImportImagesFile(context.Background(), []string{"image.png"}, "out.pdf", nil, nil)
	...
	ImportImagesFile(context.Background(), []string{"a1.png", "a2.jpg", "a3.tiff"}, "out.pdf", imp, nil)
}
...
func ExampleNUpFile() {
	...
	NUpFile(context.Background(), inFiles, "out.pdf", nil, nup, nil)
...
func ExampleSetPermissionsFile() {
	...
	SetPermissionsFile(context.Background(), "in.pdf", "", conf)
...
func ExampleEncryptFile() {
	...
	conf := model.NewAESConfiguration("upw", "opw", 256)
	EncryptFile(context.Background(), "in.pdf", "", conf)
}
...
func ExampleDecryptFile() {
	...
	conf := model.NewAESConfiguration("upw", "opw", 256)
	DecryptFile(context.Background(), "in.pdf", "", conf)
}
...
func ExampleChangeUserPasswordFile() {
	...
	conf := model.NewAESConfiguration("upw", "opw", 256)
	ChangeUserPasswordFile(context.Background(), "in.pdf", "", "upw", "upwNew", conf)
}
...
func ExampleChangeOwnerPasswordFile() {
	...
	conf := model.NewAESConfiguration("upw", "opw", 256)
	ChangeOwnerPasswordFile(context.Background(), "in.pdf", "", "opw", "opwNew", conf)
}
...
func ExampleAddAttachmentsFile() {
	...
	AddAttachmentsFile(context.Background(), "in.pdf", "", []string{"img.jpg", "attach.pdf", "test.zip"}, false, nil)
}
...
func ExampleRemoveAttachmentsFile() {
	...
	RemoveAttachmentsFile(context.Background(), "in.pdf", "", []string{"img.jpg"}, nil)

	// Remove all attachments from in.pdf
	RemoveAttachmentsFile(context.Background(), "in.pdf", "", nil, nil)
}
...
func ExampleExtractAttachmentsFile() {
	...
	ExtractAttachmentsFile(context.Background(), "in.pdf", "outDir", []string{"img.jpg"}, nil)

	// Extract all attachments from in.pdf into outDir
	ExtractAttachmentsFile(context.Background(), "in.pdf", "outDir", nil, nil)
}
...
func ExampleExtractImagesFile() {
	...
	ExtractImagesFile(context.Background(), "in.pdf", "outDir", nil, nil)
}
...
func ExampleExtractFontsFile() {
	...
	ExtractFontsFile(context.Background(), "in.pdf", "outDir", []string{"1-3"}, nil)
}
...
func ExampleExtractContentFile() {
	...
	ExtractContentFile(context.Background(), "in.pdf", "outDir", nil, nil)
}
...
func ExampleExtractPagesFile() {
	...
	ExtractPagesFile(context.Background(), "in.pdf", "outDir", []string{"even"}, nil)
}
...
func ExampleExtractMetadataFile() {
	...
	ExtractMetadataFile(context.Background(), "in.pdf", "outDir", nil)
}
